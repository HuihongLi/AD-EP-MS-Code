...
coldata <- read.csv("data/metaTG.csv", row.names=1, stringsAsFactors=FALSE)
colnames(coldata)
rownames(coldata)
...
countdata <- read.csv("data/ADmerge_remove_TG.csv", header=TRUE, row.names=1)
rownames(countdata)
colnames(countdata)
...
# We have 2 factors: age (with values 6, 8, 10, and 12) and genotype (with values WT and rTg4510)
...
# Genotype_rTg4510_vs_WT -> effect of genotype
# Age_months_8_vs_6, Age_months_10_vs_6, Age_months_12_vs_6 -> each time point compared to age 6 (effect of age)
# GenotyperTg4510.Age_months8, GenotyperTg4510.Age_months10, GenotyperTg4510.Age_months12 -> example: is GenotyperTg4510.Age_months8 different than GenotyperTg4510.Age_months6?
...
write.csv(results_Wald_frame, file = "Tg4510_res_Wald.csv") # WRONG P-VALUE (Wald test p-value: GenotyperTg4510.Age months8)
...
write.csv(results_LRT_frame, file = "Tg4510_res_LRT.csv")
...
write.csv(results_age_frame, file = "Tg4510_res_age.csv")

stats_age <- mcols(dds_age)
head(stats_age)
stats_age_frame <- data.frame(stats_age, stringsAsFactors=FALSE)

save(countdata, coldata, dds, file="DEseq2_object_counts_Tg4510.RData") # saves as objects; to open use load()
save(dds_Wald, res_Wald, stats_Wald, file="DEseq2_Wald_test_results_Tg4510.RData")
save(dds_LRT, res_LRT, stats_LRT, file="DEseq2_LRT_test_results_Tg4510.RData")
save(dds_age, res_age, stats_age, file="DEseq2_modified_LRT_test_results_Tg4510.RData")

# Stats Tables (final)
FDR_adj_genotype <- p.adjust(stats_Wald[,"WaldPvalue_Genotype_rTg4510_vs_WT"], method = "fdr") # we have calculated the FDR-adj pvalue ourselves because 1) they were not available for all analysis,and 2) from DESeq some were "NA"
FDR_adj_age <- p.adjust(stats_age[,"LRTPvalue"], method = "fdr")
FDR_adj_LRT <- p.adjust(stats_LRT[,"LRTPvalue"], method = "fdr")

stats_table <-cbind(FDR_adj_genotype, as.data.frame(stats_Wald[,c("WaldPvalue_Genotype_rTg4510_vs_WT", "Genotype_rTg4510_vs_WT")]), 
                    FDR_adj_age,
                    res_age[,"pvalue"],	
                    as.data.frame(stats_Wald[,c("WaldPvalue_Age_months_4_vs_2","Age_months_4_vs_2","WaldPvalue_Age_months_6_vs_2","Age_months_6_vs_2",
                                                "WaldPvalue_Age_months_8_vs_2","Age_months_8_vs_2")]), 
                    FDR_adj_LRT,
                    res_LRT[,"pvalue"],  
                    as.data.frame(stats_Wald[,c("WaldPvalue_GenotyperTg4510.Age_months4","GenotyperTg4510.Age_months4","WaldPvalue_GenotyperTg4510.Age_months6","GenotyperTg4510.Age_months6",
                                                "WaldPvalue_GenotyperTg4510.Age_months8","GenotyperTg4510.Age_months8")]))
rownames(stats_table) <- rownames(res_LRT)
write.csv(stats_table, file = "Tg4510_stats_table.csv")

sig_genotype <- stats_table[which(stats_table[,"FDR_adj_genotype"]<0.05),]
write.csv(sig_genotype, file = "Tg4510_sig_genotype.csv")

sig_age <- stats_table[which(stats_table[,"FDR_adj_age"]<0.05),]
write.csv(sig_age, file = "Tg4510_sig_age.csv")

sig_LRT <- stats_table[which(stats_table[,"FDR_adj_LRT"]<0.05),]
write.csv(sig_LRT, file = "Tg4510_sig_LRT.csv")

### New Table (Log2FC, etc)
## Genotype
table_publication_genotype <- cbind(FDR_adj_genotype, as.data.frame(stats_Wald[,c("WaldPvalue_Genotype_rTg4510_vs_WT", "Genotype_rTg4510_vs_WT", "WaldStatistic_Genotype_rTg4510_vs_WT")]))
...
table_publication_genotype <- table_publication_genotype[genes_Tg4510_genotype,]
head(table_publication_genotype)

write.csv(table_publication_genotype, file = "Tg4510_sig_genotype_STATS.csv")

# binomial test
threshold <- 0.05
ntotal <- nrow(table_publication_genotype) # number of sig genes I identified
ndownregulated <- nrow(table_publication_genotype[which(table_publication_genotype[,"Genotype_rTg4510_vs_WT"]<0),])
...
## Progression (LRT)
table_publication_interaction <- cbind(FDR_adj_LRT, as.data.frame(stats_LRT[,c("GenotyperTg4510.Age_months4", "GenotyperTg4510.Age_months6", "GenotyperTg4510.Age_months8", "LRTStatistic", "LRTPvalue")]))
...
table_publication_interaction <- table_publication_interaction[genes_Tg4510_interaction,]
head(table_publication_interaction)

write.csv(table_publication_interaction, file = "Tg4510_sig_interaction_STATS.csv")
...
write.csv(interaction_only, file = "Tg4510_sig_interaction-ONLY_STATS.csv")
